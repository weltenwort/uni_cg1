...
class SceneObject(object):
    def __init__(self, name="Object", position=[0, 0, 0], rotation=[0, 0, 0], offset=[0, 0, 0], scaling=[1, 1, 1], size=[1, 1, 1], color=[1, 1, 1, 1], texture=None, draw_origin=False, visible=True):
        self.name           = name
    ...
    def get_transformation(self):
        glPushMatrix()
        self.transform()
        transformation = array(glGetDoublev(GL_MODELVIEW_MATRIX)).transpose()
        glPopMatrix()
...
class TriangleMeshNode(Node):
    ...
    def update_display_list(self):
        ...
        glColor4fv(tuple(self.color))
        for geom_object in self.node_objects:
            for vertex in geom_object.vertices:
                #print(u"using normal %s" % normal_map[tuple(vertex)])
                #print(u"using texture coords %s" % str(texture_map[tuple(vertex)]))
                #mat = tuple(tuple(rand(1, 3)[0])+(1.0, ))
                #print(u"using material color %s" % str(mat))
                #glColor4fv(tuple(rand(1, 3)[0])+(0.5, ))
                #glMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, mat)
...
class ColoredPointCloudNode(Node):
    def __init__(self, points=[], *args, **kwargs):
        Node.__init__(self, *args, **kwargs)
        self.points = points
        self._dlist_geom = glGenLists(1)
    ...
    def update_display_list(self):
        ...
        glBegin(GL_POINTS)
        for point, normal, color in self.points:
            glColor4fv(tuple(color))
